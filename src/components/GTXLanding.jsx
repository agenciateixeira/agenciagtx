...
import { trackWhatsAppClick as trackWhatsAppClickServerSide } from '../lib/trackingServerSide';
...
const GTXLanding = () => {
  ...
  const trackWhatsAppSend = async () => {
    ...
    // 2. Client-side tracking (mantém o que já existia)
    if (typeof window !== 'undefined' && window.fbq) {
  ...
  const handleSendWhatsApp = async () => {
    ...
    try {
      // 1. Executa tracking (server-side + client-side)
      await trackWhatsAppSend();

      // 2. Aguarda um pouco para garantir que salvou no Supabase
